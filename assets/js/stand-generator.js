...
(function () {
  ...
  var METRIC_BOUNDS = {
    ...
    density: [10, 750], qmd: [5, 80], sd: [1, 30]
  };
  ...
  function basalAreaPerHa(result) {
    var totalM2 = 0;
    for (var i = 0; i < result.n; i++) {
      if (!result.defects.Alive[i]) continue;
  ...
  function generate() {
    try {
      ...
      var filename = 'MarkingExport-' + dateStr + '_' + timeStr + suffix + '.csv';
      ...
      var baDisplay = us ? baMetric * 4.356 : baMetric;
      var baLabel = us ? 'ft²/acre' : 'm²/ha';

      summarySizeEl.textContent = displayArea.toFixed(2) + ' ' + areaLabel +
      ...
      summaryQmdEl.textContent = displayQmd.toFixed(2) + ' ' + diamLabel + ' ± ' + displaySd.toFixed(2);
